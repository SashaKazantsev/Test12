...
function get_result_from_coocie(){
...
function loadFromCookie(){
    ...
    for (let i = 0; i<450; i++){
        cells[i].style.backdroundColor = digitToColor(code[i])
    }
}
let temp_result = get_result_from_coocie()
...
if (temp_result != '0'){
    ...
    for (let i = 0; i<450; i++){
        ...
        cell.style.backdroundColor = COLORS[parseInt(temp_result[i])]//В БУДУЮЩЕМ НАПИСАТЬ С ПОМОЩЬЮ ВЫЗОВА ФУНКЦИИ
...
setInterval(function(){
    ...
    for (let i = 0; i<temp_cells.length; i++){
        ...
        let color = cell.style.backgrundColor;
        let color_index = '0' //По умолчанию - ластик

        for(j = 0; j<COLORS.length; j++){
            element = COLORS[j]
            if (color== element){
                color_index = j.toString()
                break
            }
        ...
        document.cookie = `pixel-result=${encoded}; max-age=60000;`
    }
